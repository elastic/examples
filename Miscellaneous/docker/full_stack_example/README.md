...
1. Packetbeat is configured to use the hosts network, in order to capture traffic on the host system rather than that between the containers.  For Windows and OSX based systems, however, this host system represents the linux VM hosting docker running on Hyper-V (windows) or HyperKit(OSX). The native interface on the host system is currently not visible to Packetbeat. Improvements and contributions welcome in this area. 
